...
from claude_cost_tracker.capture import (
    QueueEntry,
    clear_queue,
    enqueue,
    queue_path,
...
def spawned(monkeypatch: pytest.MonkeyPatch) -> list[tuple[object, ...]]:
    ...
    monkeypatch.setattr(
        "claude_cost_tracker.capture.subprocess.Popen",
...
class TestEnqueue:
    def test_it_writes_a_queue_entry(self, ccost_home: Path) -> None:
    ...
    def test_it_never_analyses_inline(
        ...
        monkeypatch.setattr("claude_cost_tracker.analyse.analyse_transcript", explode)
        assert enqueue("sess-1", "/tmp/s.jsonl") == 0

    def test_repeated_hooks_append_rather_than_overwrite(self, ccost_home: Path) -> None:
        enqueue("sess-1", "/tmp/a.jsonl")
        enqueue("sess-2", "/tmp/b.jsonl")
        assert [entry.session_id for entry in read_queue()] == ["sess-1", "sess-2"]

    def test_nothing_to_enqueue_is_not_an_error(self, ccost_home: Path) -> None:
    ...
    def test_it_always_reports_success_to_the_caller(
        ...
        monkeypatch.setattr("claude_cost_tracker.capture._append", explode)
        assert enqueue("sess-1", "/tmp/s.jsonl") == 0


class TestQueueDurability:
    def test_an_unreadable_line_is_skipped_not_fatal(self, ccost_home: Path) -> None:
    ...
    def test_an_absent_queue_is_empty_not_an_error(self, ccost_home: Path) -> None:
        assert read_queue() == []

    def test_clearing_is_safe_when_there_is_nothing_to_clear(self, ccost_home: Path) -> None:
        clear_queue()
        clear_queue()

    def test_the_entry_survives_a_round_trip(self, ccost_home: Path) -> None:
        enqueue("sess-1", "/tmp/a.jsonl")
...
class TestWorkerLock:
    def test_a_live_lock_prevents_a_second_worker(
        self, ccost_home: Path, spawned: list[tuple[object, ...]]
    ) -> None:
    ...
    def test_releasing_is_safe_when_it_was_never_taken(self, ccost_home: Path) -> None:
        release_worker_lock()
        release_worker_lock()

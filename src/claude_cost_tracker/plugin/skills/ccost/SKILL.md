...
# claude-cost-tracker — where the money went
...
```sh
claude-cost-tracker                              # most recent session for this project
ccost --session <id>               # a specific session
ccost --by category                # docs vs source vs specs vs schemas
ccost --by folder                  # cost rolled up the directory tree
ccost --sort carry --top 10        # what is expensive because it is being *carried*
ccost --json                       # machine-readable, same figures
ccost explain <figure-id>          # how one number was derived, down to the records
```

Prefix with `uvx` if it is not installed: `uvx ccost`.
...
- **Tool and MCP schemas are usually the largest resident block** — measured at roughly 50× a
  project's instruction file. `ccost --by category` shows `schema` separately for this reason.

# The claude-cost-tracker Claude Code plugin
...
|---|---|
| `/ccost:audit` slash command | **Nothing.** Not in context until typed. |
| `ccost` skill | Its description only. The body loads when invoked. |
...
The hook runs `ccost _enqueue`, which appends a queue entry, spawns a detached worker, and
...
Failures are logged to `$CCOST_HOME/ccost.log` and never surfaced into the user's session
(FR-054).

## Installing

**The hook needs `ccost` on your PATH.** It runs the bare command `ccost _enqueue`, so a
copy invoked through `uvx --from git+...` — which never lands on PATH — cannot be found by it.
Install the tool first:

```sh
uv tool install --from git+https://github.com/talafek96/ccost
```

Then the plugin:

```
/plugin marketplace add talafek96/ccost
/plugin install ccost
```

Nothing analyses automatically until **both** of those are done. Until then every figure still
works — `ccost` reads the transcripts directly on each run, and the queue is only there to do

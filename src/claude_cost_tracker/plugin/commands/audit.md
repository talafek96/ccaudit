...
```sh
ccost --session "$CLAUDE_SESSION_ID"
```

If `ccost` is not installed, run it without installing:

```sh
uvx ccost --session "$CLAUDE_SESSION_ID"
```

If the session id is not available in the environment, fall back to `ccost`, which analyses

...
--     claude_cost_tracker.config.components and claude_cost_tracker.config.categories (Principle IX). A value added
...
CREATE TABLE IF NOT EXISTS injection (
    ...
    -- Must stay in sync with claude_cost_tracker.model.residency — INJECTION_CAUSES plus the attachment
...
CREATE TABLE IF NOT EXISTS attribution (
    ...
    -- The component list must stay in sync with claude_cost_tracker.config.components (Principle IX).
...
CREATE TABLE IF NOT EXISTS ingest_diagnostic (
    ...
    -- detected (claude_cost_tracker.ingest.records and claude_cost_tracker.ingest.dedup call `note(kind, sample)`).

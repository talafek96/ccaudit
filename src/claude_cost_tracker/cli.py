...
from claude_cost_tracker.analyse import (
    ReportInput,
    SessionAnalysis,
    analyse_transcript,
    contribution_of,
)
from claude_cost_tracker.capture import clear_queue, enqueue, read_queue, release_worker_lock
...
from claude_cost_tracker.config.refresh import DEFAULT_SOURCE_URL, RefreshError, refresh
from claude_cost_tracker.footprint import measure as measure_footprint
...
from claude_cost_tracker.model.policy import DEFAULT_POLICY, POLICIES
...
from claude_cost_tracker.notebook import DEFAULT_NOTEBOOK, write_notebook
...
from claude_cost_tracker.render.report import write_report
from claude_cost_tracker.render.serve import Selection, serve_ui
from claude_cost_tracker.render.terminal import build_console, render_report
from claude_cost_tracker.store.cache import cache_key, read_contribution, store_contribution
from claude_cost_tracker.store.db import SchemaVersionError, connect
from claude_cost_tracker.store.results import store_result
...
NO_CACHE_ENV = "CCOST_NO_CACHE"
# The package, not the command: `configure_logging` attaches handlers here and every other
# module logs to a child of it (`claude_cost_tracker.capture`, `.ui`). Naming this after the
# command would put it in a different tree, and those children would lose their handlers.
_LOGGER = logging.getLogger("claude_cost_tracker")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"ccost {__version__}")
    parser.add_argument(
        "-v",
        "--verbose",
        action="count",
    ...
    # `_add_analysis_options`. Without it, `ccost --project X sessions` listed the whole
    ...
    footprint_parser = subparsers.add_parser(
        "footprint",
        help="What ccost itself cost this session. It measures its own contribution.",
    )
    ...
    report_parser.add_argument(
        "--out",
        type=Path,
        default=Path("ccost-report.html"),
    ...
    notebook_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "interactively. ccost does not depend on marimo: the notebook declares what it "
            "needs inline, so `uvx marimo edit --sandbox <file>` builds a throwaway "
            "environment for it and installs nothing on your machine. Its cells call claude-cost-tracker "
    ...
    # The two selectors the notebook actually reads, so `ccost notebook --project X` parses —
    ...
    pricing = subparsers.add_parser(
        ...
        description=(
            ...
            "installed version of claude-cost-tracker. They live in a table you own, under CCOST_HOME, "
...
def _add_analysis_options(parser: argparse.ArgumentParser, *, top_level: bool = False) -> None:
    ...
    (FR-048), and so is `ccost --by category` without naming a subcommand.
    ...
    **Only the top-level copy carries the defaults.** Argparse parses a subcommand into a fresh
    ...
    filling a gap. `ccost --project X ui` set `project=X` and then had it replaced with
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"ccost: {exc}", file=sys.stderr)
        return EXIT_USAGE
    except NoSessionsFound as exc:
        print(f"ccost: {exc}", file=sys.stderr)
        return EXIT_NO_SESSIONS
    except ReconciliationError as exc:
        print(f"ccost: the breakdown does not add up: {exc}", file=sys.stderr)
        print(
            "Refusing to print figures that contradict their own total. "
            "This is a defect in claude-cost-tracker, not in your session.",
            file=sys.stderr,
        )
        ...
        print("ccost: interrupted", file=sys.stderr)
    ...
    if getattr(args, "session", None):
        ...
        if missing:
            raise NoSessionsFound(
                ...
                f"Run `ccost sessions` to see what is available."
            )
        refs = found
        ...
        if not refs:
            raise NoSessionsFound(
                "no Claude Code sessions found for this project. Run ccost from a directory "
...
def _nothing_priceable(skipped: Sequence[str]) -> str:
    ...
    return (
        ...
        f"{', '.join(skipped)}. Run `ccost pricing refresh`, or add the model to "
...
def _result_cache(*, enabled: bool = True) -> Iterator[sqlite3.Connection | None]:
    ...
    not a reason to produce nothing (FR-110). Set ``CCOST_NO_CACHE`` to skip it entirely,
    ...
    try:
        ...
        try:
            ...
            for entry in entries:
                ...
                # normalised tables and warmed nothing: a later `ccost` run still recomputed
...
def _facts_for(ref: SessionRef, args: argparse.Namespace) -> dict[str, int]:
    ...
    making it measure by default would make `ccost sessions` the slow command on a corpus.
    ...
    analysing a discovered session without that guard, and it is the one `ccost sessions
...
def _run_sessions(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        if unmeasured:
            print(
                f"ccost: {len(unmeasured)} session(s) carry no measured facts because this "
...
def _notebook_scope(args: argparse.Namespace) -> list[str]:
    """The `ccost sessions` arguments the notebook's picker selects its corpus with.
    ...
    was named, because that is what a bare `ccost sessions` lists.
    """
...
def _run_notebook(args: argparse.Namespace) -> int:
    ...
    The default is the same bargain as ``ccost ui``: one command, and nothing left behind
    ...
    if args.out is not None:
        ...
        console.print(
            ...
            f"That installs marimo into a throwaway environment for that file alone — claude-cost-tracker "
    ...
    if command is None:
        raise UsageError(
            ...
            "PATH. Install uv (https://docs.astral.sh/uv/), or run `ccost notebook --out "
    ...
    with tempfile.TemporaryDirectory(prefix="ccost-notebook-") as workspace:
        ...
        console.print(
            "Every figure in it comes from ccost itself, so it cannot show a number the "
            "terminal would not. Press Ctrl-C here when you are done."
        )
        console.print(f"Keep a copy instead with:  ccost notebook --out {DEFAULT_NOTEBOOK}")
        try:
...
def _pricing_show() -> int:
    ...
    if origin == "bundled":
        print(
            ...
            f"upgrading:\n  ccost pricing refresh\nwhich writes to {ccost_home()}."
        )
    return EXIT_OK

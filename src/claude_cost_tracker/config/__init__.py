...
2. ``$CCOST_HOME/pricing.toml`` — the user's own table. Written by ``ccost pricing
...
The refresh is explicit and opt-in (:mod:`claude_cost_tracker.config.refresh`); analysis itself never
...
from claude_cost_tracker.money import to_fraction, usd_to_micros
...
PRICING_PATH_ENV = "CCOST_PRICING"
CCOST_HOME_ENV = "CCOST_HOME"
    ...
    def staleness_note(self, today: date | None = None) -> str | None:
        ...
        if age is None:
            return (
                ...
                f"`ccost pricing refresh` to replace it with a dated one."
            )
        if age < STALE_RATES_AFTER_DAYS:
            return None
        return (
            f"These rates were published {age} days ago ({self.priced_on}) and have not been "
            f"checked since. Prices change a few times a year; run `ccost pricing refresh` "
            f"to update them without upgrading claude-cost-tracker."
        )
...
def user_pricing_path() -> Path:
    """Where ``ccost pricing refresh`` writes, and where a refreshed table is read from.

    Under ``CCOST_HOME`` rather than inside the installed package, so it survives an upgrade
    and is not owned by the release.
    """
    return ccost_home() / USER_PRICING_FILENAME


def ccost_home() -> Path:
    """The per-user state directory, created on demand — no setup step (FR-050)."""
    override = os.environ.get(CCOST_HOME_ENV)
    if override:
        return Path(override).expanduser()
    xdg = os.environ.get("XDG_STATE_HOME")
    if xdg:
        return Path(xdg).expanduser() / "claude-cost-tracker"
    return Path.home() / ".local" / "state" / "claude-cost-tracker"
...
def resolve_pricing_path() -> tuple[Path, str]:
    ...
    if explicit:
        return Path(explicit).expanduser(), "explicit ($CCOST_PRICING)"

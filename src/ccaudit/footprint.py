"""What ccaudit itself costs a session it is installed in.
...
from ccaudit.ingest.tokens import estimate_from_characters
from ccaudit.money import cost_micros

SKILL_PATH = Path(__file__).parent / "plugin" / "skills" / "ccaudit" / "SKILL.md"
...
class Footprint:
    """ccaudit's own contribution to a session, and whether it is within its own bar."""
    ...
    def lines(self) -> list[str]:
        ...
        return [
            "What ccaudit itself cost this session",
            "",
            ...
            f"  how: {self.method}",
            "",
            "  Nothing else ccaudit ships is resident when you are not using it: the slash",
...
def measure(analysis: SessionAnalysis, *, skill_path: Path | None = None) -> Footprint:
    """Measure ccaudit's own resident cost against one analysed session."""
    path = skill_path or SKILL_PATH
    description_chars = _description_length(path)
    quantity = estimate_from_characters(description_chars, "ccaudit's own skill description")
...
def _description_length(path: Path) -> int:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"cannot measure ccaudit's own footprint: {path} is missing, and the skill "

...
:func:`~ccaudit.config.ccaudit_home` on first use (FR-050). There is no config file to write
...
def database_path() -> Path:
    ...
    return ccaudit_home() / DB_FILENAME
...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    if version > SCHEMA_VERSION:
        raise SchemaVersionError(
            f"{_database_file(conn)} is at schema version {version}, but this build of ccaudit "
            f"understands version {SCHEMA_VERSION}. Upgrade ccaudit rather than reading it with "
            f"an older one."
        )
    ...
    while version < SCHEMA_VERSION:
        ...
        if statements is None:
            raise SchemaVersionError(
                ...
                f"version {version + 1} is registered in ccaudit.store.db._MIGRATIONS. Refusing "

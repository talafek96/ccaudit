...
place, under ``CCAUDIT_HOME``, surviving upgrades.

**What this does not do.** It is not on the analysis path. ``ccaudit`` analysing a session
makes no network request, needs no credential, and works fully offline (FR-029, FR-030,
SC-011); only ``ccaudit pricing refresh``, typed by the user, reaches out. Nothing about the
...
from ccaudit.money import MICROS_PER_DOLLAR, TOKENS_PER_MTOK
...
_USER_AGENT = "ccaudit-pricing-refresh"
...
def _render_toml(
    ...
    lines = [
        ...
        "#",
        "# GENERATED by `ccaudit pricing refresh`, then yours to edit. This file lives under",
        "# CCAUDIT_HOME and survives tool upgrades, so a rate you correct here stays corrected.",
        "#",
        "# Every figure ccaudit reports is API-EQUIVALENT COST imputed from these rates — never",
